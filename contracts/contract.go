...
// DeployContract Функция для развертывания контракта
func DeployContract(client *ethclient.Client, ownerKey *ecdsa.PrivateKey) (common.Address, *types.Transaction, error) {
	ctx := context.Background()

	// Получаем chainID
	chainID, err := client.NetworkID(ctx)
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при получении chain ID: %v", err)
	}
	...
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при получении nonce: %v", err)
	}

	// Получаем рекомендуемую цену газа
	gasPrice, err := client.SuggestGasPrice(ctx)
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при получении gasPrice: %v", err)
	}

	// Создаем транзактор
	auth, err := bind.NewKeyedTransactorWithChainID(ownerKey, chainID)
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при создании транзактора: %v", err)
	}
	...
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при развертывании контракта: %v", err)
	}
	...
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при ожидании подтверждения деплоя: %v", err)
	}
	if receipt.Status == 0 {
		return common.Address{}, nil, fmt.Errorf("деплой не удался (транзакция reverted)")
	}
	...
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("ошибка при проверке контракта: %v", err)
	}
...
func testLotteryInfo(client *ethclient.Client, instance *SingleLottery, contractAddress common.Address) {
	...
	if err != nil {
		logrus.Printf("Ошибка при получении цены билета: %v", err)
		return
	}

	maxTickets, err := instance.MAXTICKETS(nil)
	if err != nil {
		logrus.Printf("Ошибка при получении максимального количества билетов: %v", err)
		return
	}

	ticketsSold, err := instance.TicketsSold(nil)
	if err != nil {
		logrus.Printf("Ошибка при получении количества проданных билетов: %v", err)
		return
	}

	owner, err := instance.Owner(nil)
	if err != nil {
		logrus.Printf("Ошибка при получении владельца: %v", err)
		return
	}

	balance, err := client.BalanceAt(ctx, contractAddress, nil)
	if err != nil {
		logrus.Printf("Ошибка при получении баланса контракта: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при получении оставшихся билетов: %v", err)
		return
	}
...
func testBuyTickets(client *ethclient.Client, instance *SingleLottery, user1Key, user2Key *ecdsa.PrivateKey) {
	...
	if err != nil {
		logrus.Printf("Ошибка при получении nonce для user1: %v", err)
		return
	}
	...
	chainID, err := client.NetworkID(ctx)
	if err != nil {
		logrus.Printf("Ошибка при получении chain ID: %v", err)
		return
	}

	gasPrice, err := client.SuggestGasPrice(ctx)
	if err != nil {
		logrus.Printf("Ошибка при получении gasPrice: %v", err)
		return
	}

	auth, err := bind.NewKeyedTransactorWithChainID(user1Key, chainID)
	if err != nil {
		logrus.Printf("Ошибка при создании транзактора: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при покупке билетов: %v", err)
		return
	}
	...
	receipt, err := waitForTxReceipt(client, tx.Hash())
	if err != nil {
		logrus.Printf("Ошибка при ожидании подтверждения транзакции: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при получении nonce для user2: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при получении информации о пользователе 1: %v", err)
	} else {
...
func testDrawLottery(client *ethclient.Client, instance *SingleLottery, ownerKey *ecdsa.PrivateKey) {
	...
	nonce, err := client.PendingNonceAt(ctx, ownerAddr)
	if err != nil {
		logrus.Printf("Ошибка при получении nonce: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при проведении розыгрыша: %v", err)
		return
	}
	fmt.Printf("Транзакция отправлена: %s\n", tx.Hash().Hex())
	...
	if err != nil {
		logrus.Printf("Ошибка при проверке статуса лотереи: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при получении победителя: %v", err)
		return
	}

	winnerPrize, err := instance.WinnerPrize(nil)
	if err != nil {
		logrus.Printf("Ошибка при получении приза победителя: %v", err)
		return
	}
...
func testClaimRewards(client *ethclient.Client, instance *SingleLottery, ownerKey, user1Key, user2Key *ecdsa.PrivateKey) {
	...
	if err != nil {
		logrus.Printf("Ошибка при получении наград владельца: %v", err)
		return
	}

	user1Reward, err := instance.PendingRewards(nil, user1Addr)
	if err != nil {
		logrus.Printf("Ошибка при получении наград пользователя 1: %v", err)
		return
	}

	user2Reward, err := instance.PendingRewards(nil, user2Addr)
	if err != nil {
		logrus.Printf("Ошибка при получении наград пользователя 2: %v", err)
		return
	}
	...
	if err != nil {
		logrus.Printf("Ошибка при получении баланса владельца: %v", err)
		return
	}

	user1Balance, err := client.BalanceAt(ctx, user1Addr, nil)
	if err != nil {
		logrus.Printf("Ошибка при получении баланса пользователя 1: %v", err)
		return
	}

	user2Balance, err := client.BalanceAt(ctx, user2Addr, nil)
	if err != nil {
		logrus.Printf("Ошибка при получении баланса пользователя 2: %v", err)
		return
	}
	...
	if ownerReward.Cmp(big.NewInt(0)) > 0 {
		...
		if err != nil {
			logrus.Printf("Ошибка при клейме наград владельца: %v", err)
			return
		}
		fmt.Printf("Транзакция отправлена: %s\n", tx.Hash().Hex())
...
func claimReward(client *ethclient.Client, instance *SingleLottery, privateKey *ecdsa.PrivateKey,
	...
	if err != nil {
		logrus.Printf("Ошибка при создании транзактора для %s: %v", userLabel, err)
		return
	}
	auth.Nonce = big.NewInt(int64(nonce))
	auth.Value = big.NewInt(0)
	...
	tx, err := instance.ClaimReward(auth)
	if err != nil {
		logrus.Printf("Ошибка при клейме наград для %s: %v", userLabel, err)
		return
	}
	fmt.Printf("Транзакция отправлена: %s\n", tx.Hash().Hex())
